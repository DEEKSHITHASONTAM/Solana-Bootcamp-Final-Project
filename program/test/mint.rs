...
mod mint {
	...
	fn test_transaction_is_ok() {
		...
		let puzzle: String = Default::default();
		let description: String = Default::default();
		let hint: String = Default::default();
		...
		let (cryptopuzzle_pubkey, _) = Pubkey::find_program_address(
			&[b"cryptopuzzle", mint_info.pubkey().as_ref()],
			&program_id,
		);
		let accounts = vec![
			AccountMeta::new(fee_payer_info.pubkey(), true),
			AccountMeta::new(mint_info.pubkey(), true),
			AccountMeta::new(cryptopuzzle_pubkey, false),
