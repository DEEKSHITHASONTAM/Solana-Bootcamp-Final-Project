...
/// 2. `[writable]` cryptopuzzle: [PuzzleMetadata] 
...
/// - hint: [String] 
	pub fn process_mint(
		...
		let cryptopuzzle_info = next_account_info(account_info_iter)?;
		...
		// Derive PDAs
		let (cryptopuzzle_pubkey, cryptopuzzle_bump) = Pubkey::find_program_address(
			&[b"cryptopuzzle", mint_info.key.as_ref()],
			program_id,
		);
		...
		if *cryptopuzzle_info.key != cryptopuzzle_pubkey {
		...
		invoke_signed(
			&create_account(
				&fee_payer_info.key,
				&cryptopuzzle_info.key,
				rent_minimum_balance,
				space as u64,
				program_id,
			),
			&[fee_payer_info.clone(), cryptopuzzle_info.clone()],
			&[&[b"cryptopuzzle", mint_info.key.as_ref(), &[cryptopuzzle_bump]]],
		)?;
		...
		if cryptopuzzle_info.data_len() != PuzzleMetadata::LEN {
			return Err(NftError::InvalidAccountLen.into());
		}
		...
		let cryptopuzzle = &mut AccountPDA::new(
			&cryptopuzzle_info,
			try_from_slice_unchecked::<PuzzleMetadata>(&cryptopuzzle_info.data.borrow()).unwrap(),
			cryptopuzzle_bump,
		);
		...
		cryptopuzzle.data.serialize(&mut &mut cryptopuzzle_info.data.borrow_mut()[..])?;
...
/// 2. `[writable]` puzzle_art: [PuzzleMetadata] 
...
/// 7. `[]` csl_spl_token_v_0_0_0: [AccountInfo] Auto-generated, CslSplTokenProgram v0.0.0
	pub fn process_burn(
		...
		let fee_payer_info = next_account_info(account_info_iter)?;
		let mint_info = next_account_info(account_info_iter)?;
		let puzzle_art_info = next_account_info(account_info_iter)?;
		...
		// Derive PDAs
		let (puzzle_art_pubkey, puzzle_art_bump) = Pubkey::find_program_address(
			&[b"cryptopuzzle", mint_info.key.as_ref()],
			program_id,
		);
		...
		if *puzzle_art_info.key != puzzle_art_pubkey {
			return Err(NftError::NotExpectedAddress.into());
		}
		...
		if puzzle_art_info.data_len() != PuzzleMetadata::LEN {
			return Err(NftError::InvalidAccountLen.into());
		}
		...
		let puzzle_art = &mut AccountPDA::new(
			&puzzle_art_info,
			try_from_slice_unchecked::<PuzzleMetadata>(&puzzle_art_info.data.borrow()).unwrap(),
			puzzle_art_bump,
		);
		...
		// Accounts Serialization
		puzzle_art.data.serialize(&mut &mut puzzle_art_info.data.borrow_mut()[..])?;		
		Ok(())
	}
}