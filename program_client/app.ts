...
async function main(feePayer: Keypair) {
    ...
    /**
     * Derive the Puzzle Metadata so we can retrieve it later
     */
    const [cryptopuzzlePub] = derivePuzzleMetadataPDA(
        {
            mint: mint.publicKey,
        },
        progId,
    );
    console.info("+==== Puzzle Metadata Address ====+");
    console.info(cryptopuzzlePub.toBase58());
    ...
    await mintSendAndConfirm({
        ...
        puzzle: "Parrot",
        description: "the given parrot is macow",
        hint: "found very rarely",
        signers: {
            feePayer: feePayer,
            funding: feePayer,
            mint: mint,
    ...
    /**
     * Get the Puzzle Metadata
     */
    let cryptopuzzle = await getPuzzleMetadata(cryptopuzzlePub);
    console.info("+==== Puzzle Metadata ====+");
    console.info(cryptopuzzle);
    console.assert(cryptopuzzle!.assocAccount!.toBase58(), johnDoeATA.toBase58());
     ...
     * Get the Puzzle Metadata
     */
    cryptopuzzle = await getPuzzleMetadata(cryptopuzzlePub);
    console.info("+==== Puzzle Metadata ====+");
    console.info(cryptopuzzle);
    console.assert(cryptopuzzle!.assocAccount!.toBase58(), janeDoeATA.toBase58());
    ...
    console.info("+==== Puzzle Metadata ====+");
    console.info(cryptopuzzle);
    console.assert(cryptopuzzle!.assocAccount, "undefined");
}
